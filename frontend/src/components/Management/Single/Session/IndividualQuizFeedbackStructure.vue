<template>    
  <div class="container mt-5 col">
    ...
    <h2 v-if="linkedStep >= 0">
      O módulo está ligado a um passo. Confirme se o passo ligado tem um quiz individual
    </h2>
  ...
  methods: {
    async reset() {
      ...
      if(this.templateModuleDto) {        
        let temp = await RemoteServices.getTemplateIndividualQuizFeedbackModule(this.templateModuleDto);
      }
      if(this.packStepDto) {
        let temp = await RemoteServices.getIndividualQuizFeedbackModule(this.packStepDto);
    ...
    async putItem() {
      if(this.templateModuleDto) {        
        await RemoteServices.putTemplateIndividualQuizFeedbackModule(this.templateModuleDto);
        this.$emit('unselect');
      }
      if(this.packStepDto) {
        await RemoteServices.putIndividualQuizFeedbackModule(this.packStepDto, this.linkedStep);
        this.$emit('unselect');
      }
    },
    unselect() {
